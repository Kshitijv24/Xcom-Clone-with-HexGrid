...
{
    const float HEX_VERTICAL_OFFSET_MULTIPLIER = 0.75f;
    ...
    public GridPosition GetGridPosition(Vector3 worldPosition)
    {
        ...
        foreach (GridPosition naighbourGridPosition in neighbourGridPositionList)
        {
            if( Vector3.Distance(worldPosition, GetWorldPosition(naighbourGridPosition)) <
                    Vector3.Distance(worldPosition, GetWorldPosition(closestGridPosition)))
            {
                // Closer than the closest
                closestGridPosition = naighbourGridPosition;
            }
        }
